...
public class BroadcastHashJoinTranslator<LeftT, RightT, KeyT, OutputT>
  ...
  @Override
  public PCollection<KV<KeyT, OutputT>> translate(Join<LeftT, RightT, KeyT, OutputT> operator,
      PCollectionList<Object> inputs) {
    ...
    switch (operator.getType()) {
      case LEFT:
        final PCollection<KV<KeyT, RightT>> finalRightKeyed = rightKeyed;
        final PCollectionView<Map<KeyT, Iterable<RightT>>> broadcastRight =
            (PCollectionView<Map<KeyT, Iterable<RightT>>>)
                pViewsMap.computeIfAbsent(right, (i) -> finalRightKeyed.apply(View.asMultimap()));
        LOG.info("right = {} ", broadcastRight.hashCode());
                ...
                operator
                    ...
                    .orElseThrow(
                        () -> new IllegalStateException(
                            "Unable to infer output type descriptor.")));
      case RIGHT:
        final PCollection<KV<KeyT, LeftT>> finalLeftKeyed = leftKeyed;
        final PCollectionView<Map<KeyT, Iterable<LeftT>>> broadcastLeft =
            (PCollectionView<Map<KeyT, Iterable<LeftT>>>)
                pViewsMap.computeIfAbsent(left, (i) -> finalLeftKeyed.apply(View.asMultimap()));
